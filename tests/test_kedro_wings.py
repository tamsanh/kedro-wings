...
def test_wing_info_to_config():
    ...
    datasets = {".custom": pandas.CSVDataSet, ".missing_type": {"invalid": "dataset"}}

    wings = KedroWings(datasets)
    info = WingInfo(
        directory="dir", name="data", extension=".custom", basename="data.custom"
    )
    ...
    with pytest.raises(MissingType):
        info = WingInfo(
            directory="dir",
            name="data",
            extension=".missing_type",
            basename="data.missing_type",
        )
...
def test_kedro_wings_and_context():
            ...
            return {
                "hi": Pipeline(
                    [
                        node(
                            lambda x: x,
                            inputs="01_raw/data.csv",
                            outputs="02_intermediate/data.csv",
                        )
                    ]
                )
            }
    ...
    assert set(mock_context.catalog.list()) == {
        "01_raw/data.csv",
        "02_intermediate/data.csv",
    }
...
def test_create_chrono_entries():
    ...
    catalog_entry = wings._create_chronocode_entries([catalog_name], catalog_datasets)[
        catalog_name
    ]
